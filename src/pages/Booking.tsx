// ============================================================
// BOOKING PAGE — the heart of the site.
// Form submission: tries FormSubmit.co (no backend needed),
// falls back to mailto: so a request always reaches the inbox.
// ============================================================
...
// ---------------------------------------------
function buildEmailBody(s: FormState): string {
  const lines = [
    "NEW BOOKING REQUEST — Ohana Surf Morocco",
    "",
    `Name:          ${s.fullName}`,
    `Email:         ${s.email}`,
    `Phone:         ${s.phone}`,
    `Country:       ${s.country}`,
    `Returning?:    ${s.returning ? "Yes (10% discount)" : "No"}`,
    "",
    "— TRIP —",
    `Package:       ${s.package}`,
    `Arrival:       ${s.arrival}`,
    ...
    `Pickup:        ${s.pickup || "—"}`,
    `Dietary:       ${s.diet || "—"}`,
    `How heard:     ${s.referral || "—"}`,
    "",
    "— MESSAGE —",
    s.message || "(none)",
    "",
    `Marketing opt-in: ${s.marketing ? "Yes" : "No"}`,
  ];
  return lines.join("\n");
}
...
function BookingForm({ initialPackage }: { initialPackage?: string }) {
  const [state, setState] = React.useState<FormState>({ ...initial, package: initialPackage || "" });
  ...
  async function handleSubmit(ev: React.FormEvent) {
    ...
    // Build payload for formsubmit.co (no-backend mailer).
    // NOTE: this email must be activated by visiting the formsubmit confirmation
    // link once. Until then we silently fall back to a mailto: link.
    const payload = {
      _subject: `New booking request from ${state.fullName}`,
      ...
      "Returning guest": state.returning ? "Yes" : "No",
      Package: state.package,
      Arrival: state.arrival,
      Departure: state.departure,
      Guests: state.guests,
      "Surf level": state.level,
      Accommodation: state.accommodation || "—",
      Pickup: state.pickup || "—",
      "Dietary requirements": state.diet || "—",
      "How they heard about us": state.referral || "—",
      Message: state.message || "—",
      "Marketing opt-in": state.marketing ? "Yes" : "No",
    };

    try {
      const res = await fetch(
        `https://formsubmit.co/ajax/${encodeURIComponent(OHANA_EMAIL)}`,
        {
          method: "POST",
      ...
      // Fallback: trigger the user's email client so the request still lands
      const body = buildEmailBody(state);
      const subject = `Booking request — ${state.fullName} (${state.arrival} → ${state.departure})`;
      window.location.href = `mailto:${OHANA_EMAIL}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
      setSubmitted(true);
    } finally {
      setSubmitting(false);
    }
    ...
    return (
      <div className="booking-success">
        ...
        <div style={{marginTop:"2rem", padding:"1rem 1.25rem", background:"var(--brand-sand-100)", borderRadius:"var(--radius-md)", fontSize:"0.9rem", color:"var(--color-fg-muted)"}}>
          ...
          A copy of your request was sent to <strong>{OHANA_EMAIL}</strong>.
        </div>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
            ...
            {PICKUPS.map((p) => (
              <label key={p} className={`radio-tile radio-tile--row ${state.pickup === p ? "is-checked" : ""}`}>
                <input
                  ...
                  name="pickup"
                  value={p}
                  checked={state.pickup === p}
                  onChange={() => set("pickup", p)}
                />
                <span>{p}</span>
              </label>
            ))}
      ...
      {submitError && (
        <div style={{
          padding:"1rem", background:"#fde7e3", color:"#9b1c0d", borderRadius:"var(--radius-md)", marginTop:"1.25rem"
        }}>{submitError}</div>
      )}
...
// ---------------------------------------------
// Sidebar — contact info + reasons to book
// ---------------------------------------------
      ...
      <div style={{
        ...
        <div className="aside-row">
          ...
          <div>
            ...
            <span>We answer within 24h, usually much faster</span>
          </div>
        </div>
        <div className="aside-row">
          <Icon name="shield-check" />
          <div>
            <strong>No deposit needed</strong>
            <span>We confirm availability and quote before any payment</span>
          </div>
        </div>
        <div className="aside-row">
          <Icon name="heart" />
          <div>
            ...
            <span>10% off your next stay — tick the box in the form</span>
          </div>
        </div>
      </div>
    </aside>
...
// Page
// ---------------------------------------------
function BookingPage({ initialPackage }: { initialPackage?: string }) {
  return (
    <main>
      <PageHeader
