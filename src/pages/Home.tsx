...
// Hero with video
// -----------------------------
function Hero({ go }: { go: (r: Route) => void }) {
  ...
  return (
    <section className="hero">
      <div
        ...
        style={{
          backgroundImage:
            "url('https://images.unsplash.com/photo-1455729552865-3658a5d39692?auto=format&fit=crop&w=1920&q=80')",
        }}
      />
      <video
        ref={videoRef}
      ...
      <div className="container hero__content">
        ...
        <div className="hero__cta">
          <Button variant="primary" size="lg" iconRight="arrow-right" onClick={() => go("booking")}>
            Book Your Stay
          </Button>
          <Button variant="outline-light" size="lg" iconLeft="eye" onClick={() => go("packages")}>
            See Packages
          </Button>
        </div>
      </div>
...
// Welcome / intro
// -----------------------------
function Welcome({ go }: { go: (r: Route) => void }) {
  return (
    <Section id="welcome">
      <div className="split">
        <div className="split__copy">
          ...
          <div style={{display:"flex", gap:"0.6rem", flexWrap:"wrap", marginTop:"1.5rem"}}>
            <Button variant="teal" iconRight="arrow-right" onClick={() => go("about")}>About the camp</Button>
            <Button variant="primary" iconRight="arrow-right" onClick={() => go("booking")}>Book with us</Button>
          </div>
        </div>
...
// -----------------------------
// Package card (exported, also used on Packages page)
// -----------------------------
...
function PackagesPreview({ go }: { go: (r: Route) => void }) {
  return (
    <Section id="packages-preview" bg="sand">
      ...
      <PackagesCarousel go={go} />
      <div style={{textAlign:"center", marginTop:"2.5rem"}}>
        <Button variant="teal" size="lg" iconRight="arrow-right" onClick={() => go("packages")}>
...
// -----------------------------
// Packages carousel — shared between Home & Packages pages
// -----------------------------
function PackagesCarousel({ go, useSurfOnlyCard = false }: { go: (r: Route) => void; useSurfOnlyCard?: boolean }) {
  ...
  // SurfOnlyCard lives on the Packages page only — fall back to PackageCard when absent
  const SurfOnly = (window as any).SurfOnlyCard;
    ...
    <div>
      <div className="pkg-carousel">
        ...
        <div className="pkg-carousel__viewport">
          ...
          >
            {PACKAGES.map((pkg) => (
              useSurfOnlyCard && SurfOnly && pkg.id === "surf-only"
                ? <SurfOnly key={pkg.id} pkg={pkg} onClick={() => go("booking")} />
                : <PackageCard key={pkg.id} pkg={pkg} onClick={() => go("booking")} />
            ))}
          </div>
        </div>
...
// Day at Ohana teaser
// -----------------------------
function DailyLife({ go }: { go: (r: Route) => void }) {
  return (
    <Section>
      <div className="split split--reverse">
        <div className="split__copy">
          ...
          <Button variant="teal" iconRight="arrow-right" onClick={() => go("about")}>
            See a day at Ohana
          </Button>
        </div>
...
// Final CTA banner
// -----------------------------
function FinalCTA({ go }: { go: (r: Route) => void }) {
  return (
    <Section>
      <CTABanner
...
Object.assign(window, { HomePage, PackageCard, PackagesCarousel });
